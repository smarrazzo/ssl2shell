...
sslh [B<-F>I<config file>] [B<-t> I<num>] [B<--transparent>] [B<-p> I<listening address> [B<-p> I<listening address> ...]  [B<--tls> I<target address for TLS>] [B<--ssh> I<target address for SSH>] [B<--rvshell> I<target address for Reverse Shell>] [B<--openvpn> I<target address for OpenVPN>] [B<--http> I<target address for HTTP>] [B<--xmpp> I<target address for XMPP>] [B<--tinc> I<target address for TINC>] [B<--anyprot> I<default target address>] [B<-M> I< hex magic word>] [B<--on-timeout> I<protocol name>] [B<-u> I<username>] [B<-C> I<chroot>] [B<-P> I<pidfile>] [-v] [-i] [-V] [-f] [-n]
...
Probes for HTTP, SSL, DTLS, SSH, OpenVPN, tinc, XMPP are
...
The B<dtls> probe recognises DTLS records (RFC 6347) on UDP
listeners, such as the CoAP-over-DTLS traffic used by the
Particle/Spark cloud-server on UDP 5684. It matches a DTLS
ClientHello starting a new session as well as records from a
resumed session. It has no command-line option; enable it by
adding a protocol with C<name: "dtls"> and C<is_udp: true> to
the configuration file (see B<example.cfg>).
...
=item B<--rvshell> I<target address>

Interface and default port on which to forward Reverse Shell connections,
typically I<localhost:4444>.

=item B<-M> I<hex Magic word>

Magic hex word contained on first packet send by the reverse shell.
...
sslh [B<-F>I<config file>] [B<-t> I<num>] [B<--transparent>] [B<-p> I<listening address> [B<-p> I<listening address> ...]  [B<--tls> I<target address for TLS>] [B<--ssh> I<target address for SSH>] [B<--wireguard> I<target address for WireGuard>] [B<--openvpn> I<target address for OpenVPN>] [B<--http> I<target address for HTTP>] [B<--xmpp> I<target address for XMPP>] [B<--tinc> I<target address for TINC>] [B<--anyprot> I<default target address>] [B<--on-timeout> I<protocol name>] [B<-u> I<username>] [B<-C> I<chroot>] [B<-P> I<pidfile>] [B<-v> I<n>] [-i] [-V] [-f] [-n]
...
=item B<--wireguard> I<target address>

Interface and port on which to forward WireGuard connections,
typically I<localhost:51820>.
